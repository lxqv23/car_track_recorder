...
def group():
    global group
    choices = ["cubbies", "sparks", "tnt"]
    group = ""
    while (True):
        group = input("Load(cubbies, sparks, tnt): ").lower()
        if (group == "cubbies" or group == "sparks" or group == "tnt"):
            break
        else:
            current_msg = ("invalid input")
            # Create a label
            label = ttk.Label(tk_win, text=current_msg).grid(row=0,column=0)
...
def main():
    ...
    while True:
        ...
        if (racers_printed == False):
            current_msg = (f"Racers: {load(current_race, 0, group)},{load(current_race, 1, group)},{load(current_race, 2, group)},{load(current_race, 3, group)}")
            # Create a label
            label = ttk.Label(tk_win, text=current_msg).grid(row=0,column=0)
            racers_printed = True
        if data:
            current_msg = (str_data)
            # Create a label
            label = ttk.Label(tk_win, text=current_msg).grid(row=0,column=0)
